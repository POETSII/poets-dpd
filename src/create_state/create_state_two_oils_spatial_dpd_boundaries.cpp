...
int main(int argc, const char *argv[])
{
    ...
    builder.set_default_interaction_strengths(50.0, 5.0);

    builder.add_bead_type("W");
    builder.add_bead_type("A");
    builder.add_bead_type("B");
    builder.add_bead_type("S", true);
    ...
    std::cerr<<"Adding top boundary\n";
    builder.add_grid_2x2x2(
        "S",
        {0, 0, 0},
        {w, 1, d}
    );
    std::cerr<<"Adding left boundary\n";
    builder.add_grid_2x2x2(
        "S",
        {0, 1, 0},
        {1, h, d}
    ...
    std::cerr<<"Adding beads\n";
    for(unsigned i=1; i<=n; i++){
        
        vec3r_t x{
            1+fmod(0.5+i*a1, 1.0)*(w-1),
            1+fmod(0.5+i*a2, 1.0)*(h-1),
            fmod(0.5+i*a3, 1.0)*d
        };
        double u=udist(urng);
        if(u<0.2){
